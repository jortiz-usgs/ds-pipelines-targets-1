...
list(
  ...
  tar_target(
    model_RMSEs_csv,
    get_data(out_file = "1_fetch/out/model_RMSEs.csv"),
    format = "file"
  ), 
  ...
  tar_target(
    figure_1_png,
    draw_plot(out_file = "3_visualize/out/figure_1.png", data = eval_data), 
    format = "file"
  ),
  # Save the processed data
  tar_target(
    model_summary_results_csv,
    write_csv_return(eval_data, out_file = "2_process/out/model_summary_results.csv"), 
    format = "file"
  ),
  # Save the model diagnostics
  tar_target(
    model_diagnostic_text_txt,
    create_diag_log(out_file = "3_visualize/out/model_diagnostic_text.txt", data = eval_data), 
    format = "file"
  )
)
